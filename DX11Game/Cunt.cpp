...
// マクロ定義
#define CUNT_TEXTURE_10 L"data/texture/ato0.png"
...
#define CUNT_TEXTURE_0  L"data/texture/ato10.png"

#define BIRD_CUNT_TEXTURE_1 L"data/texture/uenotori.1.png"
#define BIRD_CUNT_TEXTURE_2 L"data/texture/uenotori.2.png"
...
Cunt::Cunt()
{
	...
	// テクスチャ読み込み
	// 鳥の残り数
	CreateTextureFromFile(pDevice, CUNT_TEXTURE_10, &g_pCuntTexture[0]);

	CreateTextureFromFile(pDevice, CUNT_TEXTURE_9, &g_pCuntTexture[1]);

	CreateTextureFromFile(pDevice, CUNT_TEXTURE_8, &g_pCuntTexture[2]);
	
    CreateTextureFromFile(pDevice, CUNT_TEXTURE_7, &g_pCuntTexture[3]);
	
    CreateTextureFromFile(pDevice, CUNT_TEXTURE_6, &g_pCuntTexture[4]);	

	CreateTextureFromFile(pDevice, CUNT_TEXTURE_5, &g_pCuntTexture[5]);
	
	CreateTextureFromFile(pDevice, CUNT_TEXTURE_4, &g_pCuntTexture[6]);

	CreateTextureFromFile(pDevice, CUNT_TEXTURE_3, &g_pCuntTexture[7]);
	
	CreateTextureFromFile(pDevice, CUNT_TEXTURE_2, &g_pCuntTexture[8]);
	
	CreateTextureFromFile(pDevice, CUNT_TEXTURE_1, &g_pCuntTexture[9]);
	
	CreateTextureFromFile(pDevice, CUNT_TEXTURE_0, &g_pCuntTexture[10]);
	...
	m_Cuntsize = XMFLOAT3(150, 50, 0);          // サイズ
	m_CuntIconPos = XMFLOAT3(550, -220, 0);		// 位置
...
void Cunt::Draw()
{
	...
	//アイコン
	SetPolygonColor(1.0f, 1.0f, 1.0f);	//ポリゴンカラーy
