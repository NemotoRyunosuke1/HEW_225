...
void UpdateModel(void)
{
	...
	if (GetKeyPress(VK_LEFT )|| GetKeyPress(VK_A) || state.Gamepad.sThumbLX < 0)
	{
	...
	else if (GetKeyPress(VK_RIGHT) || GetKeyPress(VK_D) || state.Gamepad.sThumbLX > 0)
	{
		if (GetKeyPress(VK_UP) || GetKeyPress(VK_W))
		{
			// 右前移動
	...
	// デバック用文字列
	PrintDebugProc("[ﾋｺｳｷ ｲﾁ : (%f : %f : %f)]\n", (float)state.Gamepad.sThumbLX, g_posModel.y, g_posModel.z);
