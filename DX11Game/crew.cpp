...
static TCrew		g_crew[MAX_ENEMY];	// 敵機情報
...
HRESULT InitCrew(void)
{
	...
	for (int i = 0; i < MAX_ENEMY; ++i) {
		...
		g_crew[i].m_move = XMFLOAT3(
			-SinDeg(g_crew[i].m_rot.y) * VALUE_MOVE_ENEMY,
			0.0f,
			-CosDeg(g_crew[i].m_rot.y) * VALUE_MOVE_ENEMY);
...
void UpdateCrew(void)
{
	...
	for (int i = 0; i < MAX_ENEMY; ++i) {
		...
		// 目的の角度まで慣性をかける
		g_crew[i].m_rot.y += fDiffRotY * RATE_ROTATE_ENEMY;
